...
available_models = [
    ...
    # "small", 
    #"medium",
    "large-v2" # slow, but 10x on my GTX1080
]
for model_name in tqdm(available_models):
    ...
    segments, info = model.transcribe(path, beam_size=6, language="en")

    print("Duration:", info.duration)

    predicted_segments_count = info.duration / 3
    outfile = f"C:\\Users\\J\\Desktop\\Transcripts\\transcript-{model_name}.txt"
    srtfile = outfile + ".srt"
    ...
    with open(outfile, "w", errors="ignore") as f:
        with open(srtfile, "w", errors="ignore") as srt:
            n = 0
